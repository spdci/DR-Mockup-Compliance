@method=POST @endpoint=dr/on-search
...
CRVS will call this API after processing the original search request.
    ...
    @smoke
    Scenario: Successfully receive async search results from DR
        Given SP has previously sent a search request to DR
        When DR completes processing and calls SP on-search callback
        Then SP should receive the on-search response from DR
