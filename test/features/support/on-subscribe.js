...
Given(/^SP has previously sent a subscribe request to DR$/, function () {
  specSubscribe = spec(); // Initialize the specSearch object
});

When(/^DR completes processing and calls SP on-subscribe callback$/, async function () {
  try {
...
Then(/^SP should receive the on-subscribe response from DR$/, async function () {
