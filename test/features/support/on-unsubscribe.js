...
Given(/^SP has previously sent an unsubscribe request to DR$/, function () {
  specUnsubscribe = spec(); // Initialize the specSearch object
});

When(/^DR completes processing and calls SP on-unsubscribe callback$/, async function () {
  try {
...
Then(/^SP should receive the on-unsubscribe response from DR$/, async function () {
